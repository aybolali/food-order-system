...
@Slf4j
@Component
public class OrderOutboxPaymentScheduler implements OutboxScheduler {

    private final OrderOutboxPaymentHelper orderOutboxPaymentHelper;
    private final PaymentResponseMessagePublisher paymentResponseMessagePublisher;

    public OrderOutboxPaymentScheduler(OrderOutboxPaymentHelper orderOutboxPaymentHelper,
    ...
    public void processOutboxMessage() {
        Optional<List<OrderOutboxMessage>> outboxMessagesResponse =
                orderOutboxPaymentHelper.getOrderOutboxMessageByOutboxStatus(OutboxStatus.STARTED);
        if (outboxMessagesResponse.isPresent() && outboxMessagesResponse.get().size() > 0) {
            ...
            outboxMessages.forEach(orderOutboxMessage ->
                    paymentResponseMessagePublisher.publish(orderOutboxMessage,
                            orderOutboxPaymentHelper::updateOutboxMessage));
