...
@Component
@Slf4j
//sending a data(message in avro/json model) from events to topics - same as producer
//OrderCreated
    ...
    public void publish(orderCreatedEvent domainEvent) {
        ...
        try {
            ...
            kafkaProducer.send(orderServiceConfigData.getPaymentRequestTopicName(),
                    orderId, //unique key of the kafka data
                    paymentRequestAvroModel,
                    orderKafkaMessageHelper.getKafkaCallback(orderServiceConfigData.getPaymentResponseTopicName(),  //to get results later asynchronously - (callback - in situation when we can have so much requests <-> handling a large number of requests)
                            paymentRequestAvroModel,
                            orderId,
                            "PaymentRequestAvroModel")
            );
