...
import com.food.ordering.system.restaurant.service.domain.ports.output.repository.OrderOutboxRestaurantRepository;
...
@Slf4j
@Component
public class OrderOutboxRestaurantHelper {

    private final OrderOutboxRestaurantRepository orderOutboxRestaurantRepository;
    private final ObjectMapper objectMapper;

    public OrderOutboxRestaurantHelper(OrderOutboxRestaurantRepository orderOutboxRestaurantRepository,
                                       ObjectMapper objectMapper) {
        this.orderOutboxRestaurantRepository = orderOutboxRestaurantRepository;
    ...
    public Optional<OrderOutboxMessage> getCompletedOrderOutboxMessageBySagaIdAndOutboxStatus(UUID sagaId,
                                                                                              OutboxStatus
                                                                                                      outboxStatus) {
        return orderOutboxRestaurantRepository.findByTypeAndSagaIdAndOutboxStatus(ORDER_SAGA_NAME, sagaId, outboxStatus);
    }

    @Transactional(readOnly = true)
    public Optional<List<OrderOutboxMessage>> getOrderOutboxMessageByOutboxStatus(OutboxStatus outboxStatus) {
        return orderOutboxRestaurantRepository.findByTypeAndOutboxStatus(ORDER_SAGA_NAME, outboxStatus);
    }

    @Transactional
    public void deleteOrderOutboxMessageByOutboxStatus(OutboxStatus outboxStatus) {
        orderOutboxRestaurantRepository.deleteByTypeAndOutboxStatus(ORDER_SAGA_NAME, outboxStatus);
    }
    ...
    private void save(OrderOutboxMessage orderPaymentOutboxMessage) {
        OrderOutboxMessage response = orderOutboxRestaurantRepository.save(orderPaymentOutboxMessage);
        if (response == null) {
